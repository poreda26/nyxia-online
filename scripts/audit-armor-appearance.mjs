...
const upgrade=Number(process.env.NYXIA_ARMOR_PLUS)||1;
...
await page.evaluate(async({cls,race,upgrade})=>{
...
await mkdir('output',{recursive:true});await page.locator('#audit').screenshot({path:`output/armor-${cls}-${race}-plus${upgrade}.png`});console.log(cls,race,'rendered');
}
