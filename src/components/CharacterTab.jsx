      ...
      {premiumTier && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", marginBottom: 12, borderRadius: 8, borderWidth: 1, borderStyle: "solid", borderColor: `${premiumTier.color}66`, background: `${premiumTier.color}14` }}>
          ...
          <span style={{ fontSize: 12, color: premiumTier.color, fontFamily: "var(--font-mono)" }}>
            {premiumTier.name} · {t("character.premiumDaysLeft", { days: premiumDaysLeft(player) })}
          </span>
        </div>
      )}

      <div style={styles.statsGrid}>
        <StatBlock label={t("character.stats.atk")} value={atk} color="#C9425A" />
      ...
      {subtab === "stats" && (
        <>
          <span style={styles.statPointsBadge}>
            {player.statPoints > 0 ? t("character.stats.pointsAvailable", { n: player.statPoints }) : t("character.stats.pointsNone")}
          </span>
              ...
              return (
                <div key={key} style={styles.statAllocRow}>
                  ...
                  <button
                    ...
                    title={atCap ? t("character.stats.capTitle", { cap: STAT_CAP }) : undefined}
                  >
                    <Plus size={13} />
                  </button>
                </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
                        {t(`character.skills.${skill.id}.name`)}
                        <span style={{ fontSize: 9, color: "var(--text-faint)", fontFamily: "var(--font-mono)" }}>{t("character.skills.levelTag", { level: skill.unlockLevel })}</span>
                      </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setConfirmingRespec(false)}>
          <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginTop: 14, fontFamily: "var(--font-display)", fontSize: 15, textAlign: "center", maxWidth: 240 }}>
              {t("character.stats.respec.modalPrefix")} <b>{formatGold(respecCost(player))} {t("character.stats.respec.modalGoldUnit")}</b> {t("character.stats.respec.modalSuffix")}
            </div>
            {!respecCheck.ok && (
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
              ...
              >
                {t("character.stats.respec.confirm")}
              </button>
            </div>
          </div>
        </div>
