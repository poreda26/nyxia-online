...
export default function WorldTab({ player,setPlayer,setTab,sessionRef,suspended=false }) {
  ...
  useEffect(()=>{
    let cancelled=false;
    // Optional art: a failed download must never block entering the world.
    if(player.class!=='warrior')return;
    const race=player.race==='karus'?'orc':'human';
    if(art.current.warriors?.[race])return;
    loadImage(warriorAssets[race]).then(img=>{
      const frames=measureWarriorAtlas(img);
      if(!cancelled){
        art.current.warriors={...art.current.warriors,[race]:img};
        art.current.warriorFrames={...art.current.warriorFrames,[race]:frames};
      }
    }).catch(()=>{});
    return()=>{cancelled=true;};
  },[player.class,player.race,reload]);
  const AttackIcon=player.class==='rogue'?Crosshair:Sword;
  const needsArmor=player.class==='warrior' && ['head','chest'].some(slot=>armorVariant(player.equipped[slot])!==null);
  useEffect(()=>{
    let cancelled=false;
    if(needsArmor&&!art.current.warriorArmor)loadImage(warriorArmorUrl).then(img=>{
      const frames=measureArmorAtlas(img);
      if(!cancelled){art.current.warriorArmor=img;art.current.armorFrames=frames;}
    }).catch(()=>{});
    return()=>{cancelled=true;};
  },[needsArmor,reload]);
  const weapon=player.equipped.mainHand;
  const weaponUrl=weapon?itemImageFor(weapon.name,weapon.upgradeLevel||0):null;
  const heldWeaponUrl=player.class==='warrior'?warriorWeaponUrl(weapon):null;
  useEffect(()=>{
    let cancelled=false;art.current.heldWeapon=null;
    if(heldWeaponUrl)loadImage(heldWeaponUrl).then(img=>{if(!cancelled)art.current.heldWeapon=img;}).catch(()=>{});
    return()=>{cancelled=true;};
  },[heldWeaponUrl]);
  useEffect(()=>{
