...
import { drawWarrior, weaponLight } from './warriorVisuals';
...
export function drawWorld(ctx,width,height,world,player,art) {
  ...
  for(const a of actors) {
    ...
    const modular=a.kind==='hero' && player.class==='warrior' && drawWarrior(ctx,world,player,art);
    if(!modular && a.kind==='hero' && art.animations) drawAnimatedHero(ctx,heroIndex===2?art.archer:art.animations,world,heroIndex);
    else if(!modular && art.atlas) sprite(ctx,art.atlas,a.sprite,a.x,a.y,a.kind==='hero'?88:a.kind==='npc'?84:a.sprite===5?96:82,a.facing||1);
    if(a.kind==='hero') {
      label(ctx,player.nickname,a.x,a.y-(modular?142:120),'#f8e8be',15);
      // A small equipment badge uses the actual currently equipped item asset.
      if(art.weapon && (!modular || !art.heldWeapon)) {ctx.drawImage(art.weapon,a.x+30,a.y-56,28,40);}
      if(player.class==='warrior' && world.swingUntil>world.time && world.time-world.swingStarted>=0.24 && world.time-world.swingStarted<0.40) {
        const progress=(world.time-world.swingStarted-.24)/.16;
        ctx.save();ctx.translate(a.x,a.y-44);ctx.scale(a.facing,0.55);
        const color=weaponLight(player.equipped.mainHand)||'#ffe0aa';
        ctx.globalAlpha=Math.sin(progress*Math.PI)*.8;ctx.strokeStyle=color;
        ctx.lineWidth=5;ctx.shadowBlur=12;ctx.shadowColor=color;
        ctx.beginPath();ctx.arc(0,0,attackRange(player)>100?70:55,-1.2+progress,0.3+progress);ctx.stroke();
        ctx.lineWidth=1.5;ctx.strokeStyle='#fff9e5';ctx.stroke();ctx.restore();
      }
    } else if(a.kind==='npc') {
