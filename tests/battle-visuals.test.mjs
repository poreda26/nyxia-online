...
test('each map boss keeps its regional boss artwork',()=>{
 for(const map of MAPS){
  const boss=buildMapBoss(map),art=battleVisualFor(boss);
  assert.equal(art.mapId,map.id);assert.equal(art.index,art.boss);
 }
});
